...
const BoardView = () => {
  ...
  const handleAddList = async () => {
    ...
    if (!title) return;
    const limit = prompt('Enter WIP limit (0 for no limit):', '0');
    try {
      await api.post('lists/', { title, board: id, position: 999, wip_limit: parseInt(limit) || 0 });
  ...
  const handleAiSetup = async () => {
    const promptText = prompt('Tell AI what this project is about (e.g. "E-commerce Website Build"):');
  ...
  const handleListRename = async (listId, currentTitle) => {
  ...
  const handleListColor = async (listId) => {
    const color = prompt('Enter color hex or name (e.g. #3b82f6 or blue):');
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-3.5rem)] overflow-hidden bg-white">
      {/* Board Header */}
      <div className="px-6 py-4 flex items-center justify-between border-b border-slate-100">
        <div className="flex items-center gap-4">
          ...
          <div className="flex -space-x-2">
            {(board.members || []).map((member) => (
              <div key={member.id} className="h-8 w-8 rounded-full border-2 border-white bg-blue-600 flex items-center justify-center text-white text-[10px] font-bold" title={member.username}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button 
            onClick={() => alert('Filtering coming soon! You can currently use the Search bar in the top navigation to find specific cards.')}
